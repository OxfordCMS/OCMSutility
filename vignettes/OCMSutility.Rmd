...
## top_taxa
This returns the top most represented taxa, based on cumulative relative abundance. Cumulative sums are calculated on a per-sample basis, so taxa that passes the cumulative sum threshold in any sample are retained. For example, giving the taxa that are in the top 80% abundance in any of the samples.

Usage:
Takes in dataframe or matrix of relative abundance, with samples in columns, taxa in rows. Function returns dataframe of taxa, sample_id, relative abundance, and cumulative sum of relative abundance.
